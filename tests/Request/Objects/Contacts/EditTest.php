...
class EditTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testQuery()
    {
        ...
        $testObject->contactCountry = 'testValue Country';
        ...
        $query->updateContact($testObject);

        $data = $this->api->getPostObject($query);

        $this->assertEquals('contacts', $data['objectType']);
        $this->assertEquals('edit', $data['objectAction']);
        $this->assertEquals('testValue ID', $data['values'][0]['contactID']);
        ...
        $this->assertEquals('testValue Country', $data['values'][0]['contactCountry']);
